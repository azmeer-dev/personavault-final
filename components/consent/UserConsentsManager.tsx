...
import { ExternalLink, ShieldAlert, Info, Trash2, Loader2 } from 'lucide-react';

// type definitions for expected API response
interface AppInfo {
  id: string;
  name: string;
  logoUrl?: string | null;
  ...
  return (
    <div className="space-y-4">
      {consents.map(consent => {
        const app = consent.app;
        const user = consent.requestingUser;

        if (!app && !user) return null;

        const logoSrc = app?.logoUrl ?? user?.globalProfileImage ?? undefined;
        const displayName = app?.name ?? user?.globalDisplayName ?? 'Unknown';
        const website = app?.websiteUrl ?? undefined;
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center space-x-3">
                  <Avatar className="h-12 w-12 border">
                    <AvatarImage src={logoSrc} alt={displayName} />
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {consent.grantedScopes.length > 0 ? (
