  ...
  try {
    ...
    if (!app) {
      await createAuditLog({
        ...
        actorUserId: userId,
        action: "GENERATE_API_KEY_FAILURE",
    ...
    if (app.ownerId !== userId) {
      ...
      await createAuditLog({
        ...
        outcome: AuditLogOutcome.FAILURE,
        details: {
          error: "User not owner of app",
          appName: app.name,
          actualOwner: app.ownerId,
        },
      });
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      // Typed error for better property access
      const auditActorUserId = token?.sub; // Use token.sub directly as userId might not be set if error is early
      const appNameFromFetchedApp =
        typeof app !== "undefined" && app ? app.name : undefined; // Safely access app name

      console.error(
        `[POST /api/apps/${
          params.appId
        }/keys] Error generating API key for app ${params.appId} by user ${
      ...
      if (auditActorUserId) {
        ...
        await createAuditLog({
          ...
          action: "GENERATE_API_KEY_FAILURE",
          targetEntityType: "App",
          targetEntityId: params.appId,
          outcome: AuditLogOutcome.FAILURE,
          details: {
            error: error.message || "Internal Server Error",
            appName: appNameFromFetchedApp,
            code: error.code,
          },
        });
      }
